...
positions_file <- snakemake@output[["positions"]]
...
out_dir <- dirname(true_counts_file)
...
SimulateTrueCountsOffset <- function(ncells_total,min_popsize,i_minpop=1,ngenes, 
  ...
  bandwidth <- 0.5
  positions <- matrix(runif(2 * ncells_total, min = -1, max = 1), nrow = ncells_total)
  colnames(positions) <- c('x', 'y')
  dists <- as.matrix(dist(positions))
  cov <- exp(-1 * dists / bandwidth ** 2) * Sigma * Sigma
  new_s_evfs <- t(mvrnorm(n_de_evf, rep(evf_center, ncells_total), cov))
  evfs[[3]][, (nevf - n_de_evf + 1):(nevf)] <- new_s_evfs

  gene_effects <- lapply(gene_effects, function(ge) {
      for (i in (nevf - n_de_evf + 1):(nevf)){
          ge[, i] <- (
              rnorm(nrow(ge), mean = geffect_mean, sd = gene_effects_sd) *
              rbinom(nrow(ge), 1, .05)
          )
      }
      return(ge)
  ...
  return(list(counts,gene_effects,cell_meta,params, positions))
}


# %% Follow the vignette

# Here the tree is just used as a placeholder
# EVFs are later replaced with the 2d random field
phyla <- read.tree(
...
rownames(gene_effects_df) <- gene_ids
write.table(gene_effects_df, paste0(out_dir, "/gene_effects_kon.txt"),
    sep = "\t", col.names = NA
)

gene_effects_df <- data.frame(gene_effects[[2]])
rownames(gene_effects_df) <- gene_ids
write.table(gene_effects_df, paste0(out_dir, "/gene_effects_koff.txt"),
    sep = "\t", col.names = NA
)

gene_effects_df <- data.frame(gene_effects[[3]])
rownames(gene_effects_df) <- gene_ids
write.table(gene_effects_df, paste0(out_dir, "/gene_effects_s.txt"),
    sep = "\t", col.names = NA
)
...
# Save positions too
rownames(positions) <- cell_ids
write.table(positions, positions_file, sep = "\t", row.names = TRUE)
